...
if (fs.existsSync('../lib/assets/' + info.LANG + '.json')) {var json = JSON.parse(fs.readFileSync('../lib/assets/' + info.LANG + '.json'));	} 
else { var json = JSON.parse(fs.readFileSync('../lib/assets/DEFAULT.json'));}	
...
async function updatedb() {	
	const simpleGit = require('simple-git')	
		...
		const heroku = new Heroku({ token: process.env.HKEY })	
			await git.fetch();	
					var commits = await git.log(['main' + '..origin/' +'main']);	
					if (commits.total === 0) {	
						return '```You already have latest version installed```'	
					} else {	
								var app = await heroku.get('/apps/' + process.env.HAPP)	
							 //   await Void.sendMessage(citel.chat,{text:'*ᴜᴘᴅᴀᴛɪɴɢ...*'})	
							git.fetch('upstream', 'main');	
							git.reset('hard', ['FETCH_HEAD']);	
	
							var git_url = app.git_url.replace(	
								"https://", "https://api:" + process.env.HKEY + "@"	
							)   	
							try {	
...
let fname = await huh.writeFileSync(__dirname+'/../iris_plugins/System/1.js', data, "utf8")
console.log('fname '+fname)
const command = require(__dirname+'/../iris_plugins/System/1.js', data, "utf8");
...
async function sync() {	
    ...
    const git = simpleGit();	
        await git.fetch();	
            var commits = await git.log(['main' + '..origin/' + 'main']);	
	     const { HANDLERS } = require('../info');	
            var availupdate = '```Update Avialable\n\n```';	
            commits['all'].map(	
            (commit) => { 	
...
function Igstory(username){
	return new Promise(async(resolve, reject) => {
		...
		axios.post('https://www.instagramsave.com/system/action.php',qs.stringify(Info.data), { headers: Info.headers })
		.then(({ data }) => {
		resolve(data.medias)
		   })
		})
