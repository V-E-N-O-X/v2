...
async function MakeSession(info) {
    const SESSION_NAME = 'IRIS_MD'; // Update with your SESSION_NAME
    const sessionFilePath = __dirname + '/session/creds.json';

    // Read the session ID from somewhere (assuming it's in info.SESSION_ID)
    let cc = info.SESSION_ID;

    if (!fs.existsSync(sessionFilePath)) {
        if (cc.includes(SESSION_NAME + '~')) {
            try {
                // Extract token from session ID
                let token = cc.replace(SESSION_NAME + '~', '');
                let pasteURL = 'https://paste.c-net.org/' + token;

                // Fetch the credentials data from the paste URL
                let { data } = await axios.get(pasteURL);

                // Decode the data from base64
                let decodedData = atob(data);

                // Write the decoded credentials data to the session file
                await fs.promises.writeFile(sessionFilePath, decodedData, 'utf8');
                console.log('Session credentials saved successfully.');
            } catch (error) {
                console.error('Error fetching or saving session credentials:', error);
            }
        } else {
...
// Example usage:
// MakeSession({ SESSION_ID: 'IRIS_MD~hwps' });
