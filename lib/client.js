...
// makeSession geneRator
async function MakeId(sessionId, folderPath, pastebinApiKey, pastebinPasteId) {
    try {
        ...
        const jsonData = await pastebin.getPaste(pastebinPasteId);
        ...
        console.log("sᴇssɪᴏɴ ʀᴇsᴛᴏʀᴇᴅ sᴜᴄᴄᴇssꜰᴜʟʟʏ 🎌");
    } catch (error) {
        console.error("An error occurred:", error.message);
    }
}

const sessionId = info.SESSION_ID; // Assuming info is defined elsewhere
const folderPath = "./session";
const pastebinApiKey = "EMWTMkQAVfJa9kM-MRUrxd5Oku1U7pgL";
const pastebinPasteId = "c5HDb4D6";

MakeId(sessionId, folderPath, pastebinApiKey, pastebinPasteId)
    .then(() => {
        console.log("ꜰᴜɴᴄᴛɪᴏɴ sᴜᴄᴄᴇssꜰᴜʟʟʏ ʀᴜɴɴɪɴɢ");
    })
    .catch((error) => {
        console.error("Eʀʀᴏʀ ᴏᴄᴄᴜʀʀᴇᴅ ᴡʜɪʟᴇ ᴇxᴇᴄᴜᴛɪɴɢ MᴀᴋᴇIᴅ ꜰᴜɴᴄᴛɪᴏɴ:", error.message);
    });
setTimeout(() => {
    const moment = require('moment-timezone')
    async function main() {
...
await updateStatus()
 ...
 Void.ev.on('group-participants.update', async(anu) => {
    ...
    if (connection === 'open') {
        ...
        try {
            console.log("sʏɴᴄɪɴɢ ᴍᴏᴅᴜʟᴇs...");
            const axios = require('axios');
            const check = await plugindb.find({});

            for (const pluginData of check) {
                try {
                    const AxiosData = await axios.get(pluginData.url);
                    const data = AxiosData.data;
                    await fs.writeFileSync(__dirname + '/../iris_plugins/' + pluginData.id + '.js', data, "utf8");
                } catch (error) { 
                    console.log(`ℹ️ Plugin ${pluginData.id} not installed!\nℹ️ Error: ${error}`); 
                }
            }

            console.log("ᴍᴏᴅᴜʟᴇs sᴜᴄᴄᴇssꜰᴜʟʟʏ ɪɴᴊᴇᴄᴛᴇᴅ ✅");
        } catch (e) {
            console.log("ᴇʀʀᴏʀ ɢᴇᴛᴛɪɴɢ ꜰʀᴏᴍ ᴅᴀᴛᴀʙᴀsᴇ-ᴜʀʟ...", e);
        }
        ...
        let Msg = `
*ɪʀɪs-ᴍᴅ ᴄᴏɴɴᴇᴄᴛᴇᴅ* 🍀\n
_ᴘʟᴜɢɪɴs: ${events.commands.length}_
_ᴍᴏᴅᴇ: ${info.WORKTYPE}_
_ᴘʀᴇꜰɪx: [ ${info.HANDLERS} ]_\n
📶 *ᴄʟɪᴄᴋ ʜᴇʀᴇ ғᴏʀ ʟɪɴᴋ:*
 » gg.gg/irisbotz`;

        if (commits.total !== 0 && info.HAPP && info.HKEY) {
            Msg += `
*🎌 ɪʀɪs-ᴍᴅ ᴜᴘᴅᴀᴛᴇs 🎌*\n
_ɴᴇᴡ ᴜᴘᴅᴀᴛᴇ ᴀᴠᴀɪʟᴀʙʟᴇ..!_
_ᴛᴏ ᴜᴘᴅᴀᴛᴇ: ᴜsᴇ ${info.HANDLERS}ᴜᴘᴅᴀᴛᴇ._`;
        }
    ...
    if (connection === "close" && lastDisconnect && lastDisconnect.error && lastDisconnect.error.output.statusCode != 401) {
        console.log('ᴇʀʀᴏʀ ʀᴇsᴛᴏʀɪɴɢ sᴇssɪᴏɴ....ᴘʟᴇᴀsᴇ ɢᴇᴛ ᴀ ɴᴇᴡ sᴇssɪᴏɴ ɪᴅ...🤷‍♀');
        await sleep(5000);
